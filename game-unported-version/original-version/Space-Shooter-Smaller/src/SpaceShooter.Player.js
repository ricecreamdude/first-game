SpaceShooter.Bullet = function (color) {
  ...
  this.name = 'bullet';
  this.stats = { damage: 1 };
  this.collisionList = ['enemy'];
  this.properties = { speed: { x: 0, y: -20 } };
...
SpaceShooter.Bullet.prototype.update = function (time) {
  if (false !== SpaceShooter.Element.prototype.update.call(this, time)) {
    ...
    if (this.object.position.y < -5 || this.object.position.y > renderer.height+5 || this.object.position.x < -5 || this.object.position.x > renderer.width+5) this.reset();
  }
};
...
SpaceShooter.BulletEnemyBig = function (color) {
  ...
  this.collisionList = ['ship'];
  this.properties = { speed: { x: 0, y: -30 } };
  this.radius = 10;
  this.stats = { damage: 5 };
};
...
SpaceShooter.Ship = function () {
  ...
  this.accelerationXSpeed = 3;
  this.accelerationYSpeed = 3;
  this.decelerationXSpeed = 1;
  this.decelerationYSpeed = 1;
  this.stats = { score: 0, hp: 50, damage: 5 };
  this.bulletLevel = 1;
  this.name = 'ship'; // initial name so enemies can't hit it. Will be 'ship'
  this.imuumCountdown = 300;
  this.currentTexture = 5;
  this.textures = ['smallfighter0001.png', 'smallfighter0002.png', 'smallfighter0003.png', 'smallfighter0004.png', 'smallfighter0005.png', 'smallfighter0006.png', 'smallfighter0007.png', 'smallfighter0008.png', 'smallfighter0009.png', 'smallfighter0010.png', 'smallfighter0011.png'];
...
SpaceShooter.Ship.prototype.collision = function (element) {
  if (this.imuumCountdown <= 0 && element.stats != null && element.stats.damage != null) this.damage(element.stats.damage);
  if (element.stats != null && element.stats.hp != null) element.damage(this.stats.damage);
...
SpaceShooter.Ship.prototype.damage = function (damage) {
  ...
  var color = 0x00ff00;
  if (percent < .75) color = 0xffff00;
  if (percent < .5) color = 0xff9900;
  if (percent < .25) color = 0xff0000;
...
SpaceShooter.Ship.prototype.update = function () {
  ...
  if (this.imuumCountdown > 0) {
    ...
    if (this.imuumCountdown <= 0) this.object.tint = this.originalTint;
  }
  this.object.position.y += this.speed.y;
  this.object.position.x += this.speed.x;
    ...
    if (this.bulletCounter <= 0) {
      ...
      for (var j = 1; j <= this.bulletLevel; j++) {
        for (var i = 0; i < this.bullets.length; i++) {
          ...
          if (j > this.bulletLevel || j > 10) break;
        }
      }
      if (shot == true) sounds['sound-laser'].play();
    }
  }
  var collisionObject = this.checkCollision();
  if (collisionObject != false || this.object.position.y < 0) {
    if (collisionObject != false) this.collision(collisionObject);
  }
};
