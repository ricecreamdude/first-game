SpaceShooter.Level = function () {
  ...
  this.elementsPerPlayer = .75; // The lower the faster the spawn rate. < 1
...
SpaceShooter.Level.prototype.reset = function () {
  this.nextDifficultyIncreased = 250; // Difficulty increased per this score
  this.difficultNumber = 1;
  for (var i = 0; i < players.length; i++) players[i].ship.bulletLevel = 1;
};
...
SpaceShooter.Level.prototype.spawnElement = function() {
  ...
  if (this.difficultNumber >= 1 && this.difficultNumber <= 3 || this.difficultNumber > this.maxDifficultNumber) {
    ...
    enemy.tween = new TWEEN.Tween({x: startX, y: -100}).to({ x: endX, y: (renderer.height + 100) }, 12000).onUpdate(function (p, tween) {
  ...
  if (this.difficultNumber >= 3 && this.difficultNumber <= 5 || this.difficultNumber > this.maxDifficultNumber) {
    ...
    enemy.tween = new TWEEN.Tween({x: startX, y: -100}).to({ x: [startX, endX, endX], y: [(renderer.height * .25), -100, (renderer.height + 100)] }, 15000).onUpdate(function (p, tween) {
      tween.element.object.position.x = this.x;
      tween.element.object.position.y = this.y;
      tween.element.object.rotation += tween.element.rotationSpeed;
    }).interpolation(TWEEN.Interpolation.Bezier).onComplete(function (tween) {
      tween.element.tween = null; // Is already gone in TWEEN
  ...
  if (this.difficultNumber >= 5 && this.difficultNumber <= 7 || this.difficultNumber > this.maxDifficultNumber) {
    ...
    enemy.rotationSpeed = getRandom(-.025, .025);
    enemy.tween = new TWEEN.Tween({x: startX, y: -100}).to({ x: [startX, endX, endX], y: [(renderer.height * .25), -100, (renderer.height + 100)] }, 20000).onUpdate(function (p, tween) {
      tween.element.object.position.x = this.x;
      tween.element.object.position.y = this.y;
      tween.element.object.rotation += tween.element.rotationSpeed;
    }).interpolation(TWEEN.Interpolation.Bezier).onComplete(function (tween) {
  ...
  if (this.difficultNumber>=6) {
    ...
    enemy.tween = new TWEEN.Tween({x: startX, y: -250}).to({ x: startX, y: renderer.height + 250 }, 21000).onUpdate(function (p, tween) {
      tween.element.object.position.x = this.x;
      tween.element.object.position.y = this.y;
    }).interpolation(TWEEN.Interpolation.Bezier).onComplete(function (tween) {
      tween.element.tween = null; // Is already gone in TWEEN
...
SpaceShooter.Level1 = function () {
  ...
  this.name = 'Level 1';
  this.assets = [{ name: 'background', file: 'background-far.jpg' }];
  this.sounds = [{
...
function soundMuteUnmute() {
  ...
  var mute = true;
  if (Howler._muted == true) mute = false;
  Howler.mute(mute);
